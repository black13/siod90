...
LISP consp(LISP x)
{
    if TYPEP(x,tc_cons) return(truth);
    else return(NIL);
}
...
LISP numberp(LISP x)
{
    if TYPEP(x,tc_flonum) return(truth);
    else return(NIL);
}
...
LISP eql(LISP x,LISP y)
{
    ...
    else if NTYPEP(y,tc_flonum) return(NIL);
    else if (FLONM(x) == FLONM(y)) return(truth);
    return(NIL);
}
...
LISP symbol_boundp(LISP x,LISP  env)
{
    ...
    if EQ(VCELL(x),unbound_marker) return(NIL);
    else return(truth);
}
...
LISP gc_relocate(LISP x)
{
    ...
    case tc_msubr:
        ...
        break;
    default:
        err("BUG IN GARBAGE COLLECTOR gc_relocate",NIL);
    }
    (*x).gc_mark = 1;
    CAR(x) = new;
    return(new);
...
{
    if NNULLP(args)
        if NULLP(car(args)) gc_status_flag = 0;
...
int flush_ws(FILE *f,char *eoferr)
{
    ...
    {
        c = getc(f);
        if (c == EOF) if (eoferr) err(eoferr,NIL);
            else return(c);
